...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    [SerializeField] private float maxSpeed, acceleration, deceleration, velocity, targetVelocity,
        slopeSideAngle, slopeMaxAngle, coyoteTime = 0.2f, coyoteTimeTimer, jumpBuffer = 0.2f, jumpBufferTimer, invulTimer = 1f;
    ...
    public void Move(InputAction.CallbackContext context)
    {
        ...
        {
            newVelocity.Set(maxSpeed * slopeNormalPerpendicular.x * -velocity, maxSpeed * slopeNormalPerpendicular.y * -velocity);
            rb2d.velocity = newVelocity;
        } else if (!_isGrounded)
        {
            targetVelocity = velocity * maxSpeed;
    ...
    {
        RaycastHit2D slopeHitFront = Physics2D.Raycast(checkPosition, transform.right, rayDistance, groundMask);
        RaycastHit2D slopeHitBack = Physics2D.Raycast(checkPosition, -transform.right, rayDistance, groundMask);
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(checkPosition, Vector2.down, rayDistance, groundMask);
        if (hit)
        {
